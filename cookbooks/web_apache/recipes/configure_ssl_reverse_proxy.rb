...
right_link_tag "reverse_proxy:target=https://#{node[:web_apache][:dest_fqdn]}:#{node[:web_apache][:dest_port]}" do
  action :publish
end
...
if node[:web_apache][:proxy_http] == "true"
  ...
  right_link_tag "reverse_proxy:target=http://#{node[:web_apache][:dest_fqdn]}" do
    action :publish
